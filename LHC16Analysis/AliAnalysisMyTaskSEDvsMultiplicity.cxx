...
#include "AliAnalysisMyTaskSEDvsMultiplicity.h"
...
/// \cond CLASSIMP
ClassImp(AliAnalysisMyTaskSEDvsMultiplicity);
/// \endcond

//________________________________________________________________________
AliAnalysisMyTaskSEDvsMultiplicity::AliAnalysisMyTaskSEDvsMultiplicity():
  AliAnalysisTaskSE(),
  fOutput(0),
  fListCuts(0),
  fOutputCounters(0),
...
//________________________________________________________________________
AliAnalysisMyTaskSEDvsMultiplicity::AliAnalysisMyTaskSEDvsMultiplicity(const char *name, Int_t pdgMeson,AliRDHFCuts *cuts, Bool_t switchPPb):
...
void  AliAnalysisMyTaskSEDvsMultiplicity::SetMassLimits(Double_t lowlimit, Double_t uplimit){
...
//_________________________________________________________________
void  AliAnalysisMyTaskSEDvsMultiplicity::SetMassLimits(Int_t pdg, Double_t range){
  /// set invariant mass limits
  Double_t mass=TDatabasePDG::Instance()->GetParticle(TMath::Abs(pdg))->Mass();
  SetMassLimits(mass-range,mass+range);
}
//________________________________________________________________________
void AliAnalysisMyTaskSEDvsMultiplicity::Init(){
  //
  /// Initialization
  //
...
//________________________________________________________________________
void AliAnalysisMyTaskSEDvsMultiplicity::UserCreateOutputObjects()
{
...
//________________________________________________________________________
void AliAnalysisMyTaskSEDvsMultiplicity::UserExec(Option_t */*option*/)
{
  ...
  if(!aod || !arrayCand) {
    printf("AliAnalysisMyTaskSEDvsMultiplicity::UserExec: Charm3Prong branch not found!\n");
    return;
  }
  ...
  if(vtx1){
    //cout << "vtx1->GetNContributors() =" << vtx1->GetNContributors() << endl;
    if(vtx1->GetNContributors()>0){
      fHistNEvents->Fill(1);
      isVtxOk=kTRUE;
    }
  }
  //isVtxOk=kTRUE; //override correction
  if(isVtxOk){
...
/*    else {
      AliFatal("Correction is not calculated (estimatorAvg)."); return;
    }*/
  }
 /* else {AliFatal("Correction is not calculated (isVtxOk && isDataDrivenZvtxCorr)."); return;} */
  ...
  if(fReadMC){
    ...
    if(!arrayMC) {
      printf("AliAnalysisMyTaskSEDvsMultiplicity::UserExec: MC particles branch not found!\n");
      return;
    }  
    // load MC header
    mcHeader =  (AliAODMCHeader*)aod->GetList()->FindObject(AliAODMCHeader::StdBranchName());
    if(!mcHeader) {
      printf("AliAnalysisMyTaskSEDvsMultiplicity::UserExec: MC header branch not found!\n");
      return;
    }
...
//________________________________________________________________________
void AliAnalysisMyTaskSEDvsMultiplicity::CreateImpactParameterHistos(){
...
//________________________________________________________________________
void AliAnalysisMyTaskSEDvsMultiplicity::Terminate(Option_t */*option*/)
{
  /// Terminate analysis
  //
...
Int_t AliAnalysisMyTaskSEDvsMultiplicity::CheckOrigin(TClonesArray* arrayMC, AliAODMCParticle *mcPartCandidate) const {		
  //
...
TProfile* AliAnalysisMyTaskSEDvsMultiplicity::GetEstimatorHistogram(const AliVEvent* event){
...
void AliAnalysisMyTaskSEDvsMultiplicity::CreateMeasuredNchHisto(){
...
//__________________________________________________________________________________________________
void AliAnalysisMyTaskSEDvsMultiplicity::FillMCMassHistos(TClonesArray *arrayMC, Int_t labD, Int_t countMult,Double_t nchWeight) 
{
  //
