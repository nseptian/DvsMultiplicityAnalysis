...
*/
 ...
 AliPhysicsSelectionTask *PhysicsSel = AddTaskPhysicsSelection(kFALSE,kTRUE); 
...
*/
    ...
    gROOT->LoadMacro(taskName.Data());
    AliAnalysisMyTaskSEDvsMultiplicity *dMultTask = AddTaskDvsMultiplicity(0,kFALSE,0,413,"pp13TeV_kHighMultV0","DStartoKpipiCuts_pp13TeV_kHighMultV0.root","DStartoKpipiCuts","estimators.root",11.98,kFALSE,0,AliAnalysisMyTaskSEDvsMultiplicity::kNtrk10,AliAnalysisMyTaskSEDvsMultiplicity::kEta10,kFALSE,16);
    /*
...
AliAnalysisGrid* CreateAlienHandler(TString pluginmode,Bool_t useParFiles=kFALSE, TString testfileslistWithPlugin="")
{
  ...
  //   source /tmp/gclient_env_$UID in the current shell.
   ...
   plugin->SetGridDataDir("/alice/data/2016/LHC16k"); // specify LHC period
   ...
   plugin->SetDataPattern("/pass1/AOD/*/*AliAOD.root"); // specify reco pass and AOD set
   plugin->SetFriendChainName("./AliAOD.VertexingHF.root");
   totruns += AddGoodRuns(plugin,"LHC16k"); // specify LHC period
   ...
   plugin->SetGridWorkingDir("DStarvsMultiplicityAnalysis");
   // Name of executable
   plugin->SetExecutable("DStarvsMultiplicityAnalysisLHC16k.sh");
   // Declare alien output directory. Relative to working directory.
   plugin->SetGridOutputDir("LHC16k_full_MyDStarvsMult_pp13TeV_kHighMultV0"); // In this case will be $HOME/work/output
   // Declare the analysis source files names separated by blancs. To be compiled runtime
   // using ACLiC on the worker nodes.
   plugin->SetAnalysisSource("AliAnalysisMyTaskSEDvsMultiplicity.cxx");
   // Declare all libraries (other than the default ones for the framework. These will be
   // loaded by the generated analysis macro. Add all extra files (task .cxx/.h) here.
   plugin->SetAdditionalLibs("libPWGflowBase.so libPWGflowTasks.so libPWGHFbase.so libPWGHFvertexingHF.so libGui.so libRAWDatabase.so libCDB.so libSTEER.so libTRDbase.so libPWGTRD.so AliAnalysisMyTaskSEDvsMultiplicity.cxx AliAnalysisMyTaskSEDvsMultiplicity.h");
   ...
   plugin->SetAnalysisMacro("AnalysisDStarvsMultiplicity.C");
   plugin->SetTTL(86000);
