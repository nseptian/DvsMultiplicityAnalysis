#ifndef MYALICFTASKVERTEXINGHF_H
#define MYALICFTASKVERTEXINGHF_H
/**************************************************************************
...
class MyAliCFTaskVertexingHF: public AliAnalysisTaskSE {
 public:
  ...
  MyAliCFTaskVertexingHF();
  MyAliCFTaskVertexingHF(const Char_t* name, AliRDHFCuts* cuts, TF1* func = 0x0);
  MyAliCFTaskVertexingHF& operator= (const MyAliCFTaskVertexingHF& c);
  MyAliCFTaskVertexingHF(const MyAliCFTaskVertexingHF& c);
  virtual ~MyAliCFTaskVertexingHF();
 ...
 protected:
  ...
  Int_t fYearNumber;
  /// \cond CLASSIMP     
  ClassDef(MyAliCFTaskVertexingHF,26); /// class for HF corrections as a function of many variables
  /// \endcond
};
